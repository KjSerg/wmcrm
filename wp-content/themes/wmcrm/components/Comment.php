...
class Comment {
	...
	public static function set_comment_term_to_users( $project_id, $comment_id ): array {
		$arr = [];
		error_log( 'set_comment_term_to_users' );
		...
		error_log( 'project_user_from_id ' . $project_user_from_id );
		error_log( 'project_users_to_id ' . $project_users_to_id );
		error_log( 'project_users_observer_id ' . $project_users_observer_id );
		...
		error_log( 'end set_comment_term_to_users' );
	...
	public static function the_comment_files( $comment_id ): void {
		if ( ! $discussion_files = carbon_get_post_meta( $comment_id, 'discussion_files' ) ) {
			return;
		} ?>
	...
	public static function get_comment_css_class( $comment_id, $is_read = false, $is_service = false ): string {
		$cls       = [ 'comment', 'project-section-wrapper' ];
	...
	public static function is_user_read_comment( $comment_id, $user_id ): bool {
		...
		$user_id    = intval( $user_id );
		$users_read = explode( ',', $users_read );
		$users_read = array_map( 'intval', $users_read );

		return in_array( $user_id, $users_read );
	}

	public static function get_likes( $comment_id ) {
		$likes = get_post_meta( $comment_id, '_likes', true );

		return $likes ? json_decode( $likes, true ) : [];
	}

	public static function the_comment_like_wrapper( $comment_id, $user_id ): void {
        ...
        <div class="<?php echo self::get_comment_css_class( $comment_id, $is_read, $is_service ) ?>"
            ...
            <div class="comment-head">
				...
				<?php if ( ! $is_service ): ?>
					...
					<?php endif; ?>
					<?php self::the_comment_like_wrapper( $comment_id, $user_id ); ?>
				<?php else: ?>
                    <div class=" text">
